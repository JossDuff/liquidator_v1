...
impl Reader {
    ...
    pub async fn run(&mut self) -> eyre::Result<()> {
        ...
        let searching_for_liquidatable_task = tokio::spawn(async move {
            // ignoring errors
            let _ =
                Self::search_for_liquidatable(tx3, temp_fix_comptroller_2, temp_fix_client).await;
        });
    ...
    async fn search_for_liquidatable(
        tx: Sender<Command>,
        temp_fix_comptroller: Comptroller<Provider<Ws>>,
        ...
        for (account, _) in accounts.iter() {
            ...
            if shortfall > U256::from(0) {
                ...
                for c_token_addr in assets_in.iter() {
                    ...
                    let c_token =
                        CErc20::new(Address::from(*c_token_addr), temp_fix_client.clone());
